...
mergeDatasets <- function(dir) {
  ...
  for(i in 1:length(testFilenames)) {
    ...
    cat(testFilename, ': ', ncol(currData1), "\n")
    
    currColNames <- cnames[[i]]
    #cat('currColNames: ', currColNames, "\n")
    ...
    cat('uciHarDataFrame: ', ncol(uciHarDataFrame), "\n")
  }
...
getUciHarMean <- function(uciHarDataFrame) {
  bar <- apply(foo[2:562], 2, mean)
}

getUciHarSd <- function(uciHarDataFrame) {
  bar <- apply(foo[2:562], 2, sd)
}

#5 From the data set in step 4, creates a second, independent tidy data set with the average of each variable for each activity and each subject.
getTidyUciHarMeanByActivity <- function(uciHarDataFrame) {
  g <- uciHarDataFrame[,2:ncol(uciHarDataFrame)] %>% group_by(Activity)
  c <- names(uciHarDataFrame)[2:562]  
  g %>% summarise_each(funs(mean), 1:561)
}
